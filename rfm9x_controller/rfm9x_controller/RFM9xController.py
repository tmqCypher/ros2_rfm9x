...
(433/915mhz typically).'))
        ...
        self.declare_parameter('baudrate', baudrate, ParameterDescriptor(
            ...
            description='Baud rate of the SPI connection. Default is 5mhz, but \
you might choose to lower to 1mhz if using long wires or a breadboard.'))
...
to a higher value to increase throughput or to a lower value to increase the \
likelihood of successfully received payloads. Valid values: 7800, 10400, \
15600, 20800, 31250, 41700, 62500, 125000, 250000'))
...
error correction. Try setting to a higher value to increase tolerance of \
short bursts of interference or to a lower value to increase bit rate. Valid \
values: 5, 6, 7, 8'))
...
to a higher value to increase the receiver\'s ability to distinguish signal \
from noise or to a lower value to increase the data transmission rate. Valid \
...
5 to 23 for high power devices (RFM95/96/97/98, high_power=True) or -1 to 14 \
for low power devices. The actual maximum setting for high_power=True is 20dBm \
but for values > 20 the PA_BOOST will be enabled resulting in an additional \
gain of 3dBm. The actual setting is reduced by 3dBm. The reported value will \
reflect the reduced setting.'))
...
packet. This is mainly used to prevent a hang due to a HW issue.'))
...
then only packets addressed to this node will be accepted. First byte of the \
RadioHead header.'))
...
transmissions. If 255 (0xff) then any receiving node should accept the packet. \
...
node: {response.header.node}, identifier: {response.header.identifier}, \
...
Header: destination: {destination}, node: {node}, identifier: {identifier}, \
...
def main(args=None):
    ...
    node = RMF9xController(spi, CS, RESET, 915)
